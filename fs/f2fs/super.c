...
void odiscard_wake_up_thread(void)
{
	...
	while (p != &all_f2fs_sbi) {
		...
		if (sbi->last_wp_odc_jiffies &&
			time_before(jiffies, sbi->last_wp_odc_jiffies
				+ odc_wakeup_interval)) {
			continue;
		}
...
static int f2fs_plane_notify_callback(struct notifier_block *nb,
	...
	if (evdata && evdata->data) {
		...
		if (*blank == 4) { //suspend
			...
			if (f2fs_device.battery_charging ||
				f2fs_device.battery_percent >=
				BATTERY_THRESHOLD)
				odiscard_wake_up_thread();
		} else if (*blank == 2) { //resume
...
static void default_options(struct f2fs_sb_info *sbi)
{
	...
	/* [ASTI-147]: no need to flush_merge as
	 *we have reduced most flushes.
	 */
	set_opt(sbi, FLUSH_MERGE);
#endif
	set_opt(sbi, DISCARD);
