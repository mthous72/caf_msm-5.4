...
static int select_discard_type_of2fs(struct f2fs_sb_info *sbi,
				int expect_discard_type);
#endif
				...
				block_t *blocks, unsigned int n)
{
	...
	while (pos0 < sbi->blocks_per_seg) {
		...
		pos = __find_rev_next_bit(cur_map,
				sbi->blocks_per_seg, pos0 + 1);
		blks = pos - pos0;
		order = ilog2(blks);
		if (order < n)
			blocks[order] += blks;
		total_blocks += blks;
		pos0 = __find_rev_next_zero_bit(cur_map,
				sbi->blocks_per_seg, pos + 1);
	}
	return total_blocks;
}
#endif
...
static inline bool of2fs_need_balance_dirty(struct f2fs_sb_info *sbi)
{
	...
	bd_lock(sbi);
	node_cnt = (long)(bd->curr_node_alloc_count -
		bd->last_node_alloc_count);
	data_cnt = (long)(bd->curr_data_alloc_count -
		bd->last_data_alloc_count);
...
static inline void of2fs_balance_fs(struct f2fs_sb_info *sbi)
{
	if (!sbi->gc_opt_enable) {
		/*
		 * We should do GC or end up with checkpoint,
		 *if there are so many dirty
	...
	if (!dc->bio_ref && dc->state == D_SUBMIT) {
		...
		if (dc->discard_time) {
			...
			if (discard_end_time > dc->discard_time)
				dc->discard_time = discard_end_time -
					dc->discard_time;
			else
				dc->discard_time = 0;
		}
...
bool f2fs_issue_discard_timeout(struct f2fs_sb_info *sbi)
{
	...
	/* [ASTI-147]: add for oDiscard */
	if (f2fs_odiscard_enable)
		__init_discard_policy_of2fs(sbi, &dpolicy,
			DPOLICY_UMOUNT, dcc->discard_granularity);
	else
	...
	do {
		...
		if (f2fs_odiscard_enable) {
			...
			if (dcc->odiscard_wake) {
				odiscard_expire = jiffies +
				msecs_to_jiffies(
					ODISCARD_EXEC_TIME_NO_CHARGING);
				expect_odiscard_type = DPOLICY_ODISCARD;
			}

			if (dcc->otrim_wake) {
				expect_odiscard_type = DPOLICY_FSTRIM;
				discard_cmd_cnt = atomic_read(
					&dcc->discard_cmd_cnt);
			}
			...
			if (expect_odiscard_type == DPOLICY_ODISCARD) {
				...
				if (!f2fs_device.battery_charging &&
					time_after(jiffies, odiscard_expire)) {
					wait_ms =
					DEF_MAX_DISCARD_ISSUE_TIME_OF2FS;
					continue;
				}
			}
			...
			if (sbi->gc_mode == GC_URGENT) {
				...
				__init_discard_policy_of2fs(sbi,
					&dpolicy, discard_type, 1);
			} else {
				discard_type = select_discard_type_of2fs(sbi,
					expect_odiscard_type);
				__init_discard_policy_of2fs(sbi, &dpolicy,
					discard_type,
...
int f2fs_trim_fs(struct f2fs_sb_info *sbi, struct fstrim_range *range)
{
	...
	if (f2fs_odiscard_enable)
		__init_discard_policy_of2fs(sbi, &dpolicy,
			DPOLICY_FSTRIM, cpc.trim_minlen);
	else
		__init_discard_policy(sbi, &dpolicy,
			DPOLICY_FSTRIM, cpc.trim_minlen);
#else
					...
					enum iostat_type io_type)
{
	...
	else if (fio.new_blkaddr < le32_to_cpu(
			F2FS_RAW_SUPER(sbi)->nat_blkaddr))
		bd_inc_array_val(sbi, hotcold_count, HC_META_SIT, 1);
	else if (fio.new_blkaddr < le32_to_cpu(
			F2FS_RAW_SUPER(sbi)->ssa_blkaddr))
		bd_inc_array_val(sbi, hotcold_count, HC_META_NAT, 1);
	else if (fio.new_blkaddr < le32_to_cpu(
			F2FS_RAW_SUPER(sbi)->main_blkaddr))
	...
	if (discard_type == DPOLICY_BG) {
		...
		if (utilization(sbi) > DEF_DISCARD_URGENT_UTIL) {
			...
			dpolicy->max_interval =
				DEF_MIN_DISCARD_ISSUE_TIME_OF2FS;
		}
	} else if (discard_type == DPOLICY_FORCE) {
