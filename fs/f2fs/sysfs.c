...
static int __maybe_unused frag_score_seq_show(
	struct seq_file *seq, void *offset)
{
	struct super_block *sb = seq->private;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	...
	score = total_blocks ?
		(blocks[0] + blocks[1]) * 100ULL / total_blocks : 0;
	seq_printf(seq, "%u\n", score);
	return 0;
}

static int __maybe_unused undiscard_score_seq_show(
	struct seq_file *seq, void *offset)
{
	struct super_block *sb = seq->private;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
...
static ssize_t f2fs_odiscard_enable_write(
